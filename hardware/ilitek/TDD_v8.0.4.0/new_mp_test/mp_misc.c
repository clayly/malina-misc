...
s32 Msg28xxShortTestJudge(int *deltac_data, u8 nItemID, s8 *TestFail, u16 TestFail_check[][MAX_MUTUAL_NUM])
{
	...
	for (i = 0; i < count_test_pin; i++) {
		...
		if (0 == check_thrs(deltac_data[i + (nItemID - 1) * 13], mp_test_data->sensorInfo.thrsICpin, -mp_test_data->sensorInfo.thrsICpin))
	    {
	    	TestFail[nItemID] = 1;
...
int msg28xx_short_judge(int *deltac_data)
{
    ...
    thrs = Msg28xxCovertRValue(mp_test_data->sensorInfo.thrsICpin);
...
int msg30xx_uniformity_judge(int *deltac_data, int deltac_size)
{
    ...
    int i, j, index,va_cut=13,bd_top_cut = 5,*data,*ratio,*num, sense_num, drv_num, judge, ratio_max, ratio_min;
    ...
    for(i = 0, index = 0; i <sense_num ; i++)
    {
        for(j = 0; j <drv_num ; j++, index ++)
        {
            ...
            {
                if(condition_buf[index] == NOTCH_JUDGE)
                {
                    mp_test_result->uniformity_judge.notch += golden[index];
                    uniformity_cont.notch++;
                    if(deltac_data[index+drv_num]==0)
                            deltac_data[index+drv_num]++;
                    border_ratio_buf[4*sense_num+j] = ((100* deltac_data[index])/deltac_data[index+drv_num]);
                }
                else if(i==0)//top
                {
    ...
    for(i = 0, index = 0; i < 5; i++)
    {
        for(j = 0; j <sense_num ; j++, index ++)
        {
            if(i == 0)// top
            {
                ratio_max = mp_test_data->bd_va_ratio_max.bd_top;
                ratio_min = mp_test_data->bd_va_ratio_min.bd_top;
            }
            else if(i == 1)//down
            {
                ratio_max = mp_test_data->bd_va_ratio_max.bd_bottom;
                ratio_min = mp_test_data->bd_va_ratio_min.bd_bottom;
            }
            else if(i == 2)//left
            {
                if(j <= va_cut)
                {
                    ratio_max = mp_test_data->bd_va_ratio_max.bd_l_top;
                    ratio_min = mp_test_data->bd_va_ratio_min.bd_l_top;
                }
                else
                {
                    ratio_max = mp_test_data->bd_va_ratio_max.bd_l_bottom;
                    ratio_min = mp_test_data->bd_va_ratio_min.bd_l_bottom;
                }
            }
            else if(i == 3)//right
            {
                if(j <= va_cut)
                {
                    ratio_max = mp_test_data->bd_va_ratio_max.bd_r_top;
                    ratio_min = mp_test_data->bd_va_ratio_min.bd_r_top;
                }
                else
                {
                    ratio_max = mp_test_data->bd_va_ratio_max.bd_r_bottom;
                    ratio_min = mp_test_data->bd_va_ratio_min.bd_r_bottom;
                }
            }
            else if(i == 4)//notch
            {
                ratio_max = mp_test_data->bd_va_ratio_max.notch;
                ratio_min = mp_test_data->bd_va_ratio_min.notch;
            }

            border_ratio_buf[i*sense_num+j] = msg30xx_uniformity_bd_ratio_checck(border_ratio_buf[i*sense_num+j],ratio_max,ratio_min);
